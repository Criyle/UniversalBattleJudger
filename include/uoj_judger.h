...
const RunLimit RL_COMPILER_DEFAULT = RunLimit(10, 512, 64);
...
void add_subtask_info(const int &num, const int &scr, const vector<PointInfo> &points) {
...
RunResult run_submission_program(
	...
	return run_program(
			(result_path + "/run_submission_program.txt").c_str(),
			input_file_name.c_str(),
			output_file_name.c_str(),
			"stderr",
			limit,
			("--type=" + program_type).c_str(),
			(work_path + "/" + name).c_str(),
			NULL);
}
...
RunResult run_standard_program(
	...
	string program_type = "default";
	return run_program(
			(result_path + "/run_standard_program.txt").c_str(),
			input_file_name.c_str(),
			output_file_name.c_str(),
			"/dev/null",
			limit,
